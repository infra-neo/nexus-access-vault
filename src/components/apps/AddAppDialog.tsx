...
export function AddAppDialog({ open, onOpenChange, template, onSubmit }: AddAppDialogProps) {
  ...
  useEffect(() => {
    if (template) {
      setName(template.name === 'Custom Application' ? '' : template.name);
      setUrl(template.defaultUrl);
      setConnectionMethod(template.connectionMethods[0] || 'direct');
      setResourceType(template.resourceType || 'web_app');
    }
  }, [template]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSubmit({ name, url, connectionMethod, resourceType });
    } finally {
      setLoading(false);
    }
  };

  const availableMethods = template?.connectionMethods || ['direct', 'pomerium'];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>
              {template?.name === 'Custom Application' ? 'Add Custom Application' : `Configure ${template?.name || 'Application'}`}
            </DialogTitle>
            <DialogDescription>
              Configure how users will access this application
            </DialogDescription>
          </DialogHeader>
